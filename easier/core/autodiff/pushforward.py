...
#
# NOTE this dict will be filled up by the parent `easier.core.autodiff` module
# (the parent modules import `torch_ops_manual` and `torch_ops_generated`).
pushforward_registry: Dict[Callable, Callable] = {}


# Keys are torch operators. Many operators may share the same pushforward.
#
# NOTE this dict is only filled up with explicit importing of
# `easier.core.autodiff.torch_opinfo_generated` module.
pushforward_meta: Dict[Callable, 'PushforwardMeta'] = {}


@dataclasses.dataclass
class PushforwardMeta:
    """
    EASIER devs general won't manually define an instance of PushforwardMeta,
    commonly the Meta objects are generated by `easuer/tools/gen_autodiff.py`
    from the @pushforward functions.
    """

    param_names: List[str]

    primal_result_param_pos: Union[Literal['unused'], int] = 'unused'
    ...
    # TODO does the presence of a single input tangent lead to all output
    # tangents for multi-input-multi-output ops?
