...
def dump(
    modules: List[esr.Module], dump_dir: str  # type: ignore
) -> None:
    """
...
class ElemPartInfo(JsonBase):
    ...
    # Only ElemPartArangeIdx plays a critical role on saving dump size.
...
def dump_elemparts(
    ...
    for epi, (elempart, ep_tensors_paths) in enumerate(ep_tensors.items()):
        ...
        if isinstance(elempart.idx_desc, ElemPartArangeIdx):
            ...
            # as other fine-classfieid idx_desc like ElemPartReorderedArangeIdx
            # won't save dump size, and we don't need them for JIT time
            # optimization, either.
            elempart_type = None
